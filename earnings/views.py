...
from .serializers import (
    OrderSerializer, VendorEarningsSerializer, VendorPayoutSerializer,
...
class VendorStatsView(generics.GenericAPIView):
    ...
    def get(self, request):
        ...
        pending_earnings = VendorEarnings.objects.filter(
            vendor=vendor,
            created_at__range=(start_date, end_date),
            status=VendorEarnings.STATUS_PENDING
        ).aggregate(total=Sum("net_earnings"))['total'] or 0
        
        paid_earnings = VendorEarnings.objects.filter(
            vendor=vendor,
            created_at__range=(start_date, end_date),
...
def vendor_transactions(request):
    ...
    if not vendor_id and not vendor_name:
        return Response(
            {"status": "error", "message": "Either vendor_id or vendor_name parameter is required"},
            status=status.HTTP_400_BAD_REQUEST,
        )
