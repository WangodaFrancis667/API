...
# User roles
ROLES_DATA = (("admin", "Admin"), ("vendor", "Vendor"), ("buyer", "Buyer"))
...
class PasswordReset(models.Model):
    ...
    def create_fresh(cls, user, email, validity_minutes=15):
        ...
        return cls.objects.create(
            user=user, email=email, verification_code=code, expires_at=expires_at
        )
...
class User(AbstractUser):
    ...
    class Meta:
        indexes = [
            ...
            models.Index(
                name="idx_user_wallet", fields=["wallet"]
            ),  # Fast wallet queries
            models.Index(
                name="idx_user_role_status", fields=["role", "status"]
            ),  # Composite index for common filters
            models.Index(
                name="idx_user_username_role", fields=["username", "role"]
            ),  # Fast vendor username lookups
            models.Index(
...
class UserActivityLog(models.Model):
    ...
    class Meta:
        indexes = [
            models.Index(
                name="idx_activity_user_created", fields=["user", "created_at"]
            ),
            models.Index(
                name="idx_activity_action_created", fields=["action", "created_at"]
            ),
