...
def make_eversend_request(
    ...
    for attempt in range(retry_count + 1):
        ...
        try:
            if method.upper() == "GET":
                resp = requests.get(url, headers=headers, params=data, timeout=timeout, verify=True)
            elif method.upper() == "POST":
                resp = requests.post(url, headers=headers, json=data, timeout=timeout, verify=True)
            else:
            ...
            if resp.status_code == 401 and attempt < retry_count:
                logger.warning(f"Got 401 from Eversend API, attempting token refresh (attempt {attempt + 1})")
            ...
            if attempt == retry_count:
                raise EversendAPIError(f"Request timeout after {retry_count + 1} attempts")
            time.sleep(2 ** attempt)  # Exponential backoff
            
        except requests.exceptions.HTTPError as e:
            error_msg = f"HTTP {e.response.status_code}: {e.response.text}"
            raise EversendAPIError(error_msg, e.response.status_code, e.response.json() if e.response else None)
            
        except requests.RequestException as e:
            if attempt == retry_count:
                raise EversendAPIError(f"Request failed after {retry_count + 1} attempts: {str(e)}")
