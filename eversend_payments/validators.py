...
def verify_webhook(headers: dict, raw_body: bytes) -> bool:
    ...
    if not secret:
        logger.warning("EVERSEND_WEBHOOK_SECRET not configured, skipping signature verification")
        return True
    
    # Common signature header names
    signature_header = (
        headers.get('x-eversend-signature') or 
    ...
    try:
        ...
        if not is_valid:
            logger.error(f"Webhook signature verification failed. Expected: {expected_signature}, Got: {signature}")
