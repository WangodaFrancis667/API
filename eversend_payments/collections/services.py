...
def get_collection_fees(method: str, currency: str, amount: float) -> Dict[str, Any]:
    ...
    if not all([method, currency, amount]):
        raise ValueError(
            "Missing required parameters: method, currency, and amount are required"
        )
    ...
    try:
        ...
        logger.info(
            f"Collection fees response received for {method}-{currency}-{amount}"
        )
...
def initiate_momo_transaction(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"MoMo transaction initiated successfully: {payload['transactionRef']}"
        )
        return response_data
