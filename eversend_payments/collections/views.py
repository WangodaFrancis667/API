    ...
    def post(self, request):
        ...
        try:
            ...
            if not serializer.is_valid():
                log_transaction(f"Invalid fee request from {client_ip}: {serializer.errors}", "warning")
                return Response(
            ...
            log_transaction(f"Fee request from {client_ip}: {data['method']}-{data['currency']}-{data['amount']}")
            ...
            if "data" in response_data:
                ...
                response_data["data"]["charges"] = response_data["data"].get("charges", 0) + service_fee
                response_data["data"]["total_to_pay"] = response_data["data"].get("amount", 0) + service_fee
                response_data["data"]["payable_amount"] = response_data["data"]["total_to_pay"]

            log_transaction(f"Fee calculation successful for {client_ip}")
            return Response(response_data, status=status.HTTP_200_OK)

        except ValueError as e:
            log_transaction(f"Validation error in fee request from {client_ip}: {e}", "error")
            return Response(
                {"status": "error", "message": str(e)},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            log_transaction(f"Unexpected error in fee request from {client_ip}: {e}", "error")
            return Response(
...
class CollectionOTPView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not serializer.is_valid():
                log_transaction(f"Invalid OTP request from {client_ip}: {serializer.errors}", "warning")
                return Response(
                    {"status": "error", "message": "Invalid input", "errors": serializer.errors},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            log_transaction(f"Validation error in OTP request from {client_ip}: {e}", "error")
            return Response(
                {"status": "error", "message": str(e)},
                status=status.HTTP_400_BAD_REQUEST,
            )
        except Exception as e:
            log_transaction(f"Unexpected error in OTP request from {client_ip}: {e}", "error")
            return Response(
...
class CollectionMoMoView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not serializer.is_valid():
                log_transaction(f"Invalid MoMo request from {client_ip}: {serializer.errors}", "warning")
                return Response(
                    {"status": "error", "message": "Invalid input", "errors": serializer.errors},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            data = serializer.validated_data
            log_transaction(f"MoMo transaction initiated from {client_ip}: {transaction_ref}")
            ...
            with transaction.atomic():
                ...
                txn = Transaction.objects.create(
                    ...
                    amount=Decimal(str(data["amount"])) - Decimal(str(data["service_fee"])),
                ...
                log_transaction(f"MoMo transaction processed successfully: {transaction_ref}")
            ...
            log_transaction(f"Validation error in MoMo transaction from {client_ip}: {e}", "error")
            ...
            log_transaction(f"Unexpected error in MoMo transaction from {client_ip}: {e}", "error")
            # Update transaction status to failed if it was created
            Transaction.objects.filter(transaction_ref=transaction_ref).update(status="failed")
            return Response(
                {"status": "error", "message": "MoMo transaction failed", "transactionRef": transaction_ref},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
