...
class EversendModelsTestCase(TestCase):
    ...
    def test_transaction_creation(self):
        ...
        self.assertEqual(str(transaction), f"{transaction.transaction_ref} ({transaction.status})")
    ...
    def test_wallet_unique_constraint(self):
        ...
        Wallet.objects.create(uuid='test-user-123', currency='USD', amount=Decimal('100.00'))
        
        # This should not create a duplicate
        with self.assertRaises(Exception):
            Wallet.objects.create(uuid='test-user-123', currency='USD', amount=Decimal('200.00'))
...
class EversendValidatorsTestCase(TestCase):
    ...
    def test_verify_webhook_valid_signature(self):
        ...
        payload = b'{"eventType": "wallet.load.successful", "transactionRef": "test123"}'
        secret = 'test-secret'
...
class EversendServiceTestCase(TestCase):
    ...
    @override_settings(EVERSEND_CLIENT_ID='test-id', EVERSEND_CLIENT_SECRET='test-secret')
...
class EversendCollectionsTestCase(APITestCase):
    ...
    def test_collection_fee_endpoint(self):
        ...
        with patch('eversend_payments.collections.services.get_collection_fees') as mock_fees:
