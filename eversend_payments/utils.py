...
def update_wallet_amount(*, uuid: str, currency: str, amount: Decimal, is_add: bool = True) -> bool:
    ...
    if not uuid or not currency:
        log_transaction("Invalid UUID or currency provided to update_wallet_amount", "error")
        return False
...
def update_commission(*, currency: str, service_fee: Decimal, is_add: bool = True) -> bool:
    ...
    validated_fee = validate_amount(abs(service_fee)) if service_fee != 0 else Decimal("0")
